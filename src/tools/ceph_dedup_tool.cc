...
private:
  ...
  fp_type_t get_fp_type(string fp_algo);
...
SampleDedup::fp_type_t SampleDedup::get_fp_type(string fp_algo) {
  if (fp_algo == "sha1") {
    return fp_type_t::SHA1;
  } else if (fp_algo == "sha256") {
    return fp_type_t::SHA256;
  } else if (fp_algo == "sha512") {
    return fp_type_t::SHA512;
  } else {
    return fp_type_t::UNKNOWN;
  }
}
...
void SampleDedup::flush_duplicable_object(ObjectItem& object) {
  ...
  int ret = io_ctx.operate(
      ...
      NULL);
  if (ret == -EINVAL) {
    ret = make_manifest_object_and_flush(object.oid, io_ctx, chunk_io_ctx);
    if (ret < 0) {
